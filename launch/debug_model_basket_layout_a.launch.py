"""
Launch the RB-VOGUI basket layout A model in a package-local Gazebo debug world.
...
def generate_launch_description() -> LaunchDescription:
    """Launch the RB-VOGUI basket layout A model for debugging and inspection."""
    actions: list[LaunchDescriptionEntity] = [
        ...
        DeclareLaunchArgument(
            ...
            default_value=PathJoinSubstitution(
                [
                    FindPackageShare('robot_rbvogui_basket_layout_a'),
                    'config',
                    'default_params.yaml',
                ]
            ),
...
def _launch_actions_after_world_ready(
    ...
    if event.returncode != 0:
        ...
        get_logger('robot_rbvogui_basket_layout_a').error(reason)
...
def _include_render_robot_urdf() -> IncludeLaunchDescription:
    """Render the basket layout A Xacro before starting child processes."""
...
def _include_spawn_world() -> IncludeLaunchDescription:
    """Start the shared Gazebo world used to inspect the basket layout A model."""
    return IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution(
...
def _launch_rviz() -> Node:
    ...
    return Node(
        ...
        arguments=[
            '-d',
            PathJoinSubstitution(
                [FindPackageShare('robot_rbvogui_basket_layout_a'), 'rviz', 'sim_debug.rviz']
            ),
        ],
        output='both',
